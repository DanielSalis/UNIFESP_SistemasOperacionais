...
PUBLIC int execLottery()
{
	...
	int winner;
	int total_tickets = 0;
	int current_number_of_proccess = NR_PROCS

		for (proc_nr = 0, rmp = schedproc; proc_nr < current_number_of_proccess; proc_nr++, rmp++)
	{
		if ((rmp->flags & IN_USE) && rmp->priority == 15)
		{
...
int do_noquantum(message *m_ptr)
{
	...
	if ((rv = schedule_process_local(rmp) != OK) && (rv = execLottery() != OK))
	{
		return rv;
	}
	return OK;
